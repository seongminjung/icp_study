...
우선 outlier rejection을 위해 두 점 사이의 거리가 상위 10%인 쌍을 매칭에서 제외된다. 이후 아래의 수식을 이용하여 현재 프레임과 직전 프레임의 무게중심을 계산한다. 이 단계에서는 수직선과 평면의 높이를 가중치로 취하지 않는 것이 정확도가 높은 것으로 나타났다.
...
위 그래프는 앞의 세 방식에 대해 프레임별 소요 시간과 정확도를 나타낸다. Outlier들이 있기는 하나, 대부분의 구간에서 본 연구에서 제안한 알고리즘이 가장 빠른 속도를 보였으며, 특히 Sequence 초반에 sliding 현상이 크게 발생하는 구간에서 더욱 빠른 속도와 높은 정확도를 보였다.
