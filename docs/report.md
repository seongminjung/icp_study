...
평면 병합을 통해 단순히 직전 프레임의 평면만을 이용하는 것이 아닌 여러 프레임의 평면 정보를 종합하여 더 넓은 평면을 추출하거나 오차를 보정할 수 있다. 평면 병합을 하지 않은 경우 오차율이 더 높은 것을 위 표를 통해 확인할 수 있다.
...
첫 번째로, 각 time step마다 추정한 pose와 ground truth pose 사이의 거리를 비교하였다.
